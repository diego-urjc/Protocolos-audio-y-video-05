...
async def main():
    if len(sys.argv) < 2:
        print("Uso: python client_video_udp.py <nombre_servidor>")
        return

    requested_server = sys.argv[1]
    ...
    offer_message = {"type": "offer", "sdp": pc.localDescription.sdp, "client_port": local_port, "server": requested_server}

    loop = asyncio.get_event_loop()
    transport, protocol = await loop.create_datagram_endpoint(
        lambda: ClientProtocol(offer_message, video_done_future),
